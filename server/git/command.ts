...
export function parseCommitFileStats(output: string): Array<{ file: string; additions: number; deletions: number }> {
  return output
    .split('\n')
...
export function workingTreeDiffArgs(area: import('../types.js').WorkingTreeDiffArea, subcommand: '--numstat' | ''): string[] {
  if (area === 'staged') {
...
export async function runGit(args: string[], repoPath: string, env?: NodeJS.ProcessEnv): Promise<string> {
