...
describe("appTabs", () => {
  ...
  test("findRepositoryForTab resolves repository tabs and ignores fixed tabs", () => {
    ...
    expect(getRepositoryTabPath(getRepositoryTabId(alphaRepository.path))).toBe(
  ...
  test("restores active tabs against available repositories and derives stub names from paths", () => {
    ...
    expect(resolveRestoredActiveTabId([alphaRepository, betaRepository], CONFIG_TAB_ID)).toBe(
      CONFIG_TAB_ID,
    );
    expect(
      resolveRestoredActiveTabId(
    ...
    ).toBe(getRepositoryTabId(betaRepository.path));
    expect(
      resolveRestoredActiveTabId([alphaRepository], getRepositoryTabId(betaRepository.path)),
    ).toBe(getRepositoryTabId(alphaRepository.path));
  });
