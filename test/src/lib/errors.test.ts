...
describe("describeGitError", () => {
  ...
  test("maps missing upstream for pull", () => {
    const parsed = describeGitError(
      "Current branch 'main' has no upstream branch.",
      "Pull に失敗しました。",
    );

    expect(parsed.title).toBe("upstream branch が未設定です");
  });

  test("maps ff-only pull rejection", () => {
    const parsed = describeGitError(
      "fatal: Not possible to fast-forward, aborting.",
      "Pull に失敗しました。",
    );

    expect(parsed.title).toBe("fast-forward で pull できません");
  });
  ...
  test("maps merge overwrite message from pull", () => {
    const parsed = describeGitError(
