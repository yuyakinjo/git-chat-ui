...
import type { StashEntry, WorkingTreeStatus } from '../../../src/types';
...
describe('GitOperationPanel', () => {
  ...
  test('renders change buckets and commit controls in a shared responsive grid', () => {
    ...
    expect(html).toContain('flex flex-1 items-center justify-center gap-2 text-center text-sm font-semibold text-ink-soft');
    ...
    expect(html).toContain('git-operation-panel__stacked-bucket git-operation-panel__stacked-bucket--staged');
    expect(html).toContain('git-operation-panel__stacked-bucket git-operation-panel__stacked-bucket--stash');
