...
describe("generateCommitTitle", () => {
  ...
  test("uses only the selected provider when both tokens are configured", async () => {
    const calls: Array<[string, RequestInit | undefined]> = [];
    ...
    globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("anthropic.com")) {
        return new Response(
          JSON.stringify({ content: [{ type: "text", text: "fix: claude result" }] }),
          {
            status: 200,
            headers: { "Content-Type": "application/json" },
          },
  ...
  test("does not fallback to the other provider when the selected provider fails", async () => {
    const calls: Array<[string, RequestInit | undefined]> = [];
...
describe("listOpenAiModels", () => {
  test("returns sorted model ids with the default model first", async () => {
    ...
    await expect(listOpenAiModels("sk-openai-valid")).resolves.toEqual([
      "gpt-4.1-mini",
      "gpt-4.1",
      "o4-mini",
    ]);
