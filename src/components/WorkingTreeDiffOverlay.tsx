      ...
      <section className="panel flex h-full min-h-0 flex-col overflow-hidden p-4 shadow-2xl">
        <div className="mb-3 flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="diff-overlay__title truncate text-base font-semibold">
              {resolvedFilePath || "Changed file"}
            </div>
            <div className="diff-overlay__meta mt-1 flex flex-wrap items-center gap-2 text-xs">
              ...
              <span
                className={`badge ${area === "staged" ? "bg-[#ecfdf3]! text-[#157347]!" : "bg-[#fff4d6]! text-[#a15c00]!"}`}
              >
                {areaLabel}
              </span>
        ...
        {loading ? (
          <div className="diff-overlay__muted p-4 text-sm">差分を読み込み中...</div>
        ) : null}

        {!loading && !detail ? (
          <div className="diff-overlay__muted p-4 text-sm">差分を表示できませんでした。</div>
        ) : null}
