...
- セッション開始直後に `bun run codex:tasks:bootstrap` を実行し、`tasks/threads` の同期と watcher 起動を済ませてから作業に入る
...
1. **まず計画を立てる**：チェック可能な項目として `tasks/threads/{threadId}/todo.md` に計画を書く
2. **計画を確認する**：実装を開始する前に確認する
3. **進捗を記録する**：完了した項目を随時マークしていく
4. **変更を説明する**：各ステップで高レベルのサマリーを提供する
5. **結果をドキュメント化する**：`tasks/threads/{threadId}/todo.md` にレビューセクションを追加する
...
- ユーザーに選択肢を提示する時は、AskUserQuestionToolを使う
